...
import fitz  # PyMuPDF
import os
TOKEN = os.getenv("TOKEN", "939712b15705")
MY_NUMBER = os.getenv("MY_NUMBER", "918081335178")  # Insert your number {91}{Your number}
...
async def resume() -> str:
    ...
    """
    resume_path = Path(os.getenv("RESUME_PATH", "Resume__1_ (19).pdf")) # Replace "your_resume.pdf" with your actual filename

    if not resume_path.is_file():
        raise McpError(ErrorData(code=INTERNAL_ERROR, message="Resume file not found at " + str(resume_path)))
    ...
    try:
        ...
        # You can use markdownify to convert plain text to markdown,
        # but for simple text extraction, it's not strictly necessary.
        # However, if your PDF contains HTML-like elements, it might be useful.
        # For a PDF, the extracted text is usually already in a plain format,
        # so direct conversion is often the most reliable method.
        # In this case, we'll simply return the extracted text.

        return resume_text.strip() # .strip() removes leading/trailing whitespace

    except Exception as e:
        raise McpError(ErrorData(code=INTERNAL_ERROR, message=f"Failed to process resume file: {e!r}"))
